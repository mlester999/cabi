...
export async function maybeSummarizeConversation(profileId: string, conversationId: string, provider: AIProvider, config: ProviderConfig) {
  const db = getServiceClient(); if (!db) return false;
  const { data: owned } = await db.from("conversations").select("id").eq("id", conversationId).eq("user_id", profileId).maybeSingle(); if (!owned) return false;
  ...
  const { data: messages } = await db.from("messages").select("id,role,content,created_at").eq("conversation_id", conversationId).in("role", ["user", "assistant"]).eq("status", "complete").order("created_at", { ascending: true }).limit(80);
