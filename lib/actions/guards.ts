...
const baseSchema = {
  ...
  debugDetails: z.object({
    requestId: z.string().max(100),
    source: z.enum(["ADMIN_TEST", "CHAT_GENERATION", "HTTP_GENERATION"]),
    wallet: z.string().max(40).nullable(),
    conversation: z.string().max(40).nullable(),
    provider: z.string().max(80).nullable(),
    model: z.string().max(160).nullable(),
    referenceVersion: z.number().int().min(0).nullable(),
    referenceAttached: z.boolean(),
    aspectRatio: z.string().max(12).nullable(),
    ...
    lastStage: z.string().max(80).nullable(),
    httpStatus: z.number().int().min(100).max(599).nullable(),
    contentType: z.string().max(80).nullable(),
    byteLength: z.number().int().min(0).max(50_000_000).nullable(),
    error: z.string().max(100).nullable(),
    latencyMs: z.number().int().min(0).max(900_000),
