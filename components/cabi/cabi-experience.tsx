...
function PresenceArt({ mood, speaking = false, authenticated, bond, className = "" }: { mood: string; speaking?: boolean; authenticated: boolean; bond: Bond | null; className?: string }) {
  ...
  return (
    <div className={`relative flex min-h-0 ${className || "h-[clamp(300px,42vh,500px)]"} flex-none items-end justify-center overflow-hidden rounded-[28px] border border-white/[0.055] bg-[#0b0912]`}>
      ...
      {!failed ? (
        ...
        <div className="cabi-breathe relative z-10 mb-24 grid h-64 w-64 place-items-center rounded-full border border-violet-200/10 bg-gradient-to-b from-violet-300/10 to-transparent text-center shadow-[0_0_100px_rgba(139,92,246,.12)]">
          <div><MiniCabi className="mx-auto h-28 w-28 rounded-[26px]" priority /><div className="mt-3 text-xs font-semibold uppercase tracking-[0.28em] text-violet-300/60">Cat Partner Unit</div></div>
        </div>
      )}
...
export function CabiExperience({ flags = defaultFeatureFlags, viewport = "full", cpuGateBypassed = false, statusMessages }: {
  ...
  return (
      ...
      <div className="grid h-full min-h-0 grid-cols-[256px_minmax(420px,1fr)_360px] max-xl:grid-cols-[232px_minmax(420px,1fr)_320px] max-lg:grid-cols-[232px_minmax(0,1fr)] max-md:grid-cols-1">
      ...
      <section id="cabi-chat" className="relative flex min-h-0 min-w-0 flex-col bg-[#08070d]/55" tabIndex={-1}>
          <header className="flex h-[70px] shrink-0 items-center border-b border-white/[0.06] px-4 max-sm:h-[64px] max-sm:px-3"><button onClick={() => { setSidebarOpen(true); setProfileOpen(false); }} className="focus-ring mr-2 hidden h-11 w-11 place-items-center rounded-xl text-[#a8a3b3] hover:bg-white/[0.04] max-md:grid" aria-label="Open menu"><Menu size={20} /></button><button onClick={() => setProfileOpen(true)} className="focus-ring mr-3 hidden rounded-[14px] max-lg:block" aria-label="Open Cabi profile"><MiniCabi className="h-9 w-9" /></button><div className="min-w-0 flex-1"><div className="flex items-center gap-2"><h1 className="text-[15px] font-semibold">Cabi</h1><span className="relative h-2 w-2 rounded-full bg-emerald-300"><span className="absolute inset-0 animate-ping rounded-full bg-emerald-300/50" /></span></div>{sending ? (
              // The rotating Cabi status replaces the static line while she works,
              // so the header itself is the loader rather than a separate spinner.
              <CabiActivityStatus type="CHAT_THINKING" overrides={statusMessages} compact className="mt-0.5" />
            ) : (
              <p className="truncate text-xs text-[#777180]">{status}{temporaryChat && wallet.authenticated ? " · temporary chat" : ""}</p>
