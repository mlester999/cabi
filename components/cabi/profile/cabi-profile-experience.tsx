      ...
      <div className="mx-auto w-full max-w-[1000px] px-4 py-6 sm:px-7 sm:py-9">
        <header className="flex items-center gap-3 sm:gap-4">
          ...
          <div className="min-w-0 flex-1">
            ...
            <p className="mt-1 truncate text-sm text-[var(--cabi-text-muted)]">Cat Partner Unit</p>
          </div>
        ...
        <div className="mt-8 grid gap-5 lg:grid-cols-[minmax(0,1fr)_minmax(0,1fr)]">
          <section className="glass relative overflow-hidden rounded-2xl p-5">
            ...
            <div className="mt-3 flex flex-wrap items-center justify-between gap-3">
              <div>
                <p className="text-[10px] font-semibold uppercase tracking-[.18em] text-[var(--cabi-primary)]">Mood</p>
                <p className="mt-1 text-lg font-semibold" style={{ color: mood.accent }}>{mood.label}</p>
              </div>
              <p className="max-w-[16rem] text-right text-[12px] leading-5 text-[var(--cabi-text-secondary)]">{mood.status}</p>
            </div>
          </section>
          ...
          <div className="space-y-5">
            {view.phase === "loading" && (
              <div className="space-y-3" aria-busy="true" aria-label="Loading Cabi profile">
                {[0, 1, 2].map((index) => <div key={index} className="h-28 animate-pulse rounded-2xl border border-[var(--cabi-hairline)] bg-[var(--cabi-surface-1)]" />)}
              </div>
            )}

            {view.phase === "disconnected" && (
              <section className="glass rounded-2xl p-7">
                <span className="grid h-12 w-12 place-items-center rounded-2xl border border-violet-200/15 bg-violet-300/[0.06] text-[var(--cabi-primary)]"><Heart size={20} /></span>
              ...
              <>
                <section className="glass rounded-2xl p-6">
                  <div className="flex items-start justify-between gap-4">
                    <div>
                      <p className="text-[10px] font-semibold uppercase tracking-[.18em] text-[var(--cabi-primary)]">Bond level {profile.level}</p>
                      <h2 className="mt-2 text-2xl font-semibold tracking-[-.03em]">{profile.label}</h2>
                    </div>
                    <Heart size={20} className="text-[var(--cabi-primary)]" fill="currentColor" aria-hidden="true" />
                  </div>
                  <div className="mt-5 h-2 overflow-hidden rounded-full bg-[var(--cabi-surface-3)]">
                  ...
                  <p className="mt-3 text-xs text-[var(--cabi-text-muted)]">
                    {profile.level >= 10
                ...
                <section className="glass rounded-2xl p-6">
                  <p className="text-[10px] font-semibold uppercase tracking-[.18em] text-[var(--cabi-primary)]">Together</p>
                  ...
                  <p className="mt-4 flex items-start gap-2 text-[11px] leading-5 text-[var(--cabi-text-muted)]">
                ...
                <div className="flex flex-wrap gap-2">
                  <Link href="/settings/memory" className="focus-ring inline-flex h-11 items-center gap-2 rounded-xl border border-[var(--cabi-hairline)] bg-[var(--cabi-surface-2)] px-4 text-sm font-semibold text-[var(--cabi-text-secondary)] hover:bg-[var(--cabi-surface-3)]">
                    <Sparkles size={15} /> {profile.memoryCount} {profile.memoryCount === 1 ? "memory" : "memories"}
                  </Link>
                  <Link href="/" className="focus-ring inline-flex h-11 items-center gap-2 rounded-xl bg-[var(--cabi-primary)] px-4 text-sm font-semibold text-[var(--cabi-on-primary)] hover:brightness-105">
                    <MessageCircleMore size={15} /> Talk to Cabi
                  </Link>
                  <button onClick={() => void load()} className="focus-ring inline-flex h-11 items-center gap-2 rounded-xl border border-[var(--cabi-hairline)] bg-[var(--cabi-surface-2)] px-4 text-sm font-semibold text-[var(--cabi-text-secondary)] hover:bg-[var(--cabi-surface-3)]">
...
function Stat({ label, value }: { label: string; value: string }) {
  return (
    <div className="rounded-2xl border border-[var(--cabi-hairline)] bg-[var(--cabi-surface-1)] p-4">
      <dt className="text-[10px] uppercase tracking-[.14em] text-[var(--cabi-text-muted)]">{label}</dt>
