    ...
    <main className="cabi-noise min-h-[100dvh] overflow-x-hidden bg-transparent text-white">
      <div className="mx-auto w-full max-w-[820px] px-4 py-6 sm:px-7 sm:py-9">
        <header className="flex items-center gap-3 sm:gap-4">
          <Link href="/cabi" className="focus-ring grid h-11 w-11 shrink-0 place-items-center rounded-xl border border-[var(--cabi-hairline)] bg-[var(--cabi-surface-2)] text-[var(--cabi-text-secondary)] hover:text-white" aria-label="Back to Cabi profile">
            <ArrowLeft size={18} />
          </Link>
          <MiniCabi className="h-12 w-12" />
          <div className="min-w-0 flex-1">
            <h1 className="text-2xl font-semibold tracking-[-.03em]">Memory</h1>
            <p className="mt-1 truncate text-sm text-[var(--cabi-text-muted)]">What Cabi remembers about you.</p>
          </div>
        ...
        {view.phase === "loading" && (
          <div className="mt-8 space-y-3" aria-busy="true" aria-label="Loading memories">
            ...
            {[0, 1, 2].map((index) => <div key={index} className="h-20 animate-pulse rounded-2xl border border-[var(--cabi-hairline)] bg-[var(--cabi-surface-1)]" />)}
          </div>
        )}

        {view.phase === "disconnected" && (
          <section className="glass mt-10 rounded-2xl p-7 text-center">
            <span className="mx-auto grid h-12 w-12 place-items-center rounded-2xl border border-violet-200/15 bg-violet-300/[0.06] text-[var(--cabi-primary)]"><Brain size={20} /></span>
          ...
          <div className="mt-8 space-y-4">
            <section className="glass flex flex-wrap items-center justify-between gap-4 rounded-2xl p-5">
              <div className="min-w-0">
                ...
                <p className="mt-1 max-w-md text-xs leading-5 text-[var(--cabi-text-muted)]">When this is off, she stops using saved details and stops saving new ones. Existing memories stay until you delete them.</p>
              </div>
            ...
            <section className="glass rounded-2xl p-5">
              <div className="flex flex-wrap items-center justify-between gap-3">
                <p className="text-[10px] font-semibold uppercase tracking-[.18em] text-[var(--cabi-primary)]">
                  {view.memories.length} {view.memories.length === 1 ? "memory" : "memories"}
                </p>
                <div className="flex items-center gap-2">
                  <button onClick={() => void load()} className="focus-ring inline-flex h-9 items-center gap-1.5 rounded-xl border border-[var(--cabi-hairline)] bg-[var(--cabi-surface-2)] px-3 text-[11px] font-semibold text-[var(--cabi-text-secondary)] hover:bg-[var(--cabi-surface-3)]">
              ...
              {view.memories.length === 0 ? (
                <div className="mt-5 rounded-2xl border border-dashed border-[var(--cabi-hairline)] p-8 text-center">
                  <MiniCabi className="mx-auto h-12 w-12 opacity-70" decorative />
                  <p className="mt-3 text-sm text-[var(--cabi-text-muted)]">Nothing saved yet.</p>
                  <p className="mt-1 text-xs text-[var(--cabi-text-faint)]">Say &ldquo;remember that I like small-cap AI coins&rdquo; and it will show up here.</p>
                </div>
              ) : (
                <ul className="mt-4 space-y-2">
                  {view.memories.map((memory) => (
                    <li key={memory.id} className="flex items-start gap-3 rounded-2xl border border-[var(--cabi-hairline)] bg-[var(--cabi-surface-1)] p-4">
                      <div className="min-w-0 flex-1">
                        <p className="text-[10px] font-semibold uppercase tracking-[.12em] text-[var(--cabi-primary)]">{memory.category}</p>
                        <p className="mt-1 break-words text-sm leading-6 text-[var(--cabi-text-secondary)]">{memory.content}</p>
                        <p className="mt-1.5 text-[10px] text-[var(--cabi-text-faint)]">{new Date(memory.created_at).toLocaleDateString()}</p>
                      </div>
                      <button onClick={() => void forget(memory.id)} className="focus-ring grid h-9 w-10 shrink-0 place-items-center rounded-xl text-[var(--cabi-text-muted)] hover:bg-rose-300/[0.06] hover:text-rose-300" aria-label={`Forget: ${memory.content.slice(0, 40)}`}>
                        <Trash2 size={16} />
                      </button>
                    </li>
                  ))}
            ...
            <p className="flex items-start gap-2 px-1 text-[11px] leading-5 text-[var(--cabi-text-faint)]">
              <Sparkles size={13} className="mt-0.5 shrink-0 text-[var(--cabi-primary)]" aria-hidden="true" />
            ...
            {notice && <p role="status" className="px-1 text-xs text-[var(--cabi-primary)]">{notice}</p>}
          </div>
        )}
      </div>
    </main>
