...
export function AdminDataTable({ resource, title, description, rowHref }: {
  ...
  return <div><Header eyebrow="Admin records" title={title} description={description} />{!databaseReady && <Banner>Connect Supabase to view live records.</Banner>}<p className="mt-6 text-xs text-[var(--cabi-text-muted)]">{count} total records · viewing the newest 50</p><div className="scrollbar-cabi mt-3 overflow-x-auto rounded-2xl border border-[var(--cabi-hairline)] bg-[var(--cabi-surface)]"><table className="w-full min-w-[680px] text-left text-sm"><thead className="border-b border-[var(--cabi-hairline)] bg-[var(--cabi-surface-1)] text-[10px] uppercase tracking-[.11em] text-[var(--cabi-text-muted)]"><tr>{columns.map((column) => <th key={column} className="px-4 py-3 font-semibold">{column.replaceAll("_", " ")}</th>)}</tr></thead><tbody className="divide-y divide-white/[0.05]">{rows.map((row, index) => { const href = rowHref?.(row) ?? null; return <tr key={String(row.id ?? index)} className="hover:bg-[var(--cabi-surface-1)]">{columns.map((column) => <td key={column} className="max-w-[280px] truncate px-4 py-3 text-xs text-[var(--cabi-text-secondary)]" title={String(row[column] ?? "")}>{format(row[column])}</td>)}{href && <td className="px-4 py-3 text-right"><Link href={href} className="focus-ring rounded-lg px-2 py-1 text-xs font-semibold text-[var(--cabi-primary)] hover:bg-[var(--cabi-surface-3)]">Open</Link></td>}</tr>; })}</tbody></table>{!rows.length && <div className="p-10 text-center text-sm text-[var(--cabi-text-muted)]">No records yet.</div>}</div></div>;
}
