...
export function ImageCardView({ card, onRegenerate, onUseAsAvatar, status, onRetry, onDelete, onShare }: {
  ...
  return (
    <figure
      ...
      className="mt-3 overflow-hidden rounded-2xl border border-violet-200/[0.14] bg-[var(--cabi-surface-1)]"
    >
      {status === "QUEUED" || status === "GENERATING" ? (
        ...
        <div className="grid aspect-square w-full place-items-center bg-gradient-to-br from-violet-500/[0.08] to-transparent p-6 text-center">
          <div className="w-full max-w-[16rem]">
            <MiniCabi className="cabi-breathe mx-auto h-20 w-20 rounded-2xl" decorative />
            <CabiActivityStatus type="IMAGE_GENERATING" className="mt-4 justify-center" mascot={false} />
            <p className="mt-1 text-[11px] text-[var(--cabi-text-muted)]">This usually takes a few seconds.</p>
          </div>
        </div>
      ) : status === "FAILED" ? (
        <div className="grid aspect-square w-full place-items-center bg-black/40 p-6 text-center">
          <div>
            <p className="text-[13px] font-semibold text-[var(--cabi-text-secondary)]">{cabiFailureMessages.IMAGE}</p>
            {onRetry ? (
              <button
                type="button"
                onClick={() => onRetry(card.prompt)}
                className="focus-ring mt-4 inline-flex h-9 items-center gap-1.5 rounded-xl border border-[var(--cabi-hairline)] bg-[var(--cabi-surface-2)] px-4 text-[11px] font-semibold text-[var(--cabi-text-secondary)] hover:bg-[var(--cabi-surface-3)]"
              >
      ...
      <figcaption className="p-3.5">
        <p className="flex items-center gap-2 text-[12px] text-[var(--cabi-text-secondary)]">
          <Sparkles size={12} className="shrink-0 text-[var(--cabi-primary)]" aria-hidden="true" />
          <span className="min-w-0 flex-1 truncate">{card.prompt}</span>
        </p>

        {card.xp ? <p className="mt-1.5 text-[11px] font-semibold text-[var(--cabi-primary)]">First image today +{card.xp} XP</p> : null}
        ...
        <div className="mt-3 flex flex-wrap gap-2">
          ...
          {onDelete ? (
            <button
              type="button"
              onClick={onDelete}
              className="focus-ring grid h-9 w-9 place-items-center rounded-xl border border-[var(--cabi-hairline)] bg-[var(--cabi-surface-2)] text-[var(--cabi-text-muted)] hover:text-rose-200"
