    ...
    <section className={`mt-3 w-full max-w-[min(100%,34rem)] overflow-hidden rounded-xl border ${accent} p-4 shadow-[0_10px_30px_rgba(0,0,0,.28)] ${className}`} aria-label={`${card.title} card`}>
      ...
      {card.debugDetails && (
        <details className="mt-3 rounded-lg border border-violet-200/[0.12] bg-black/20 px-2.5 py-2 text-[10px] text-[var(--cabi-text-muted)]">
          <summary className="cabi-focus cursor-pointer text-[10px] font-semibold text-[var(--cabi-text-secondary)]">[Debug details]</summary>
          <dl className="mt-2 grid grid-cols-2 gap-x-3 gap-y-1">
            ...
            <div><dt className="uppercase tracking-[.08em] text-white/35">Error</dt><dd>{card.debugDetails.error ?? "—"}</dd></div>
            ...
            <div><dt className="uppercase tracking-[.08em] text-white/35">Wallet</dt><dd className="font-mono">{card.debugDetails.wallet ?? "—"}</dd></div>
          </dl>
