...
export function PreviewBanner({ mode, actor }: { mode: PreviewChromeMode; actor: { kind: "admin" | "wallet"; label: string } }) {
  ...
  return (
    <div className="sticky top-0 z-[80] shrink-0 border-b border-amber-300/15 bg-[var(--cabi-bg-raised)]/92 backdrop-blur-xl">
      <div className="mx-auto flex min-h-12 w-full max-w-[1400px] flex-wrap items-center gap-x-4 gap-y-2 px-4 py-2 sm:px-6">
        ...
        <p className="min-w-0 flex-1 text-[11px] leading-5 text-[var(--cabi-text-secondary)]">
          {modeCopy[mode]} <span className="hidden sm:inline">Signed in as {actor.label}.</span>
        </p>
        <div className="flex items-center gap-2">
          <Link
            href="/admin"
            className="focus-ring flex h-8 items-center gap-1.5 rounded-lg border border-[var(--cabi-hairline)] bg-[var(--cabi-surface-2)] px-2.5 text-[11px] font-semibold text-[var(--cabi-text-secondary)] transition hover:bg-[var(--cabi-surface-3)]"
          >
          ...
          <button
            ...
            onClick={() => void exit()}
            disabled={busy}
            className="focus-ring flex h-8 items-center gap-1.5 rounded-lg bg-amber-100 px-2.5 text-[11px] font-semibold text-[var(--cabi-on-primary)] transition hover:bg-amber-50 disabled:opacity-60"
          >
      ...
      <p className="mx-auto flex w-full max-w-[1400px] items-center gap-2 px-4 pb-2 text-[10px] leading-4 text-[var(--cabi-text-muted)] sm:px-6">
