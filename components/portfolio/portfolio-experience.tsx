      ...
      <div className="mx-auto w-full max-w-[1000px] px-4 py-6 sm:px-7 sm:py-9">
        <header className="flex items-center gap-3 sm:gap-4">
          ...
          <div className="min-w-0 flex-1">
            ...
            <p className="mt-1 truncate text-sm text-[var(--cabi-text-muted)]">What your wallet holds on the network Cabi is set to.</p>
          </div>
        ...
        {view.phase === "loading" && (
          <div className="mt-8 space-y-3" aria-busy="true" aria-label="Loading portfolio">
            {[0, 1, 2].map((index) => <div key={index} className="h-24 animate-pulse rounded-2xl border border-[var(--cabi-hairline)] bg-[var(--cabi-surface-1)]" />)}
          </div>
        )}

        {view.phase === "disconnected" && (
          <section className="glass mt-10 rounded-2xl p-7 text-center sm:p-10">
            <span className="mx-auto grid h-12 w-12 place-items-center rounded-2xl border border-violet-200/15 bg-violet-300/[0.06] text-[var(--cabi-primary)]"><Wallet size={20} /></span>
            <h2 className="mt-5 text-xl font-semibold">Connect a wallet first</h2>
            <p className="mx-auto mt-3 max-w-md text-sm leading-7 text-[var(--cabi-text-secondary)]">
            ...
            <button onClick={wallet.openConnect} className="focus-ring mt-6 h-11 rounded-xl bg-white px-5 text-sm font-semibold text-[var(--cabi-on-primary)]">Connect Wallet</button>
          </section>
        )}
        ...
        {view.phase === "error" && (
          <section className="glass mt-10 rounded-2xl p-7 text-center sm:p-10">
            ...
            <p className="mx-auto mt-3 max-w-md text-sm leading-7 text-[var(--cabi-text-secondary)]">{view.message}</p>
            <button onClick={() => void load({ fresh: true })} className="focus-ring mt-6 inline-flex h-11 items-center gap-2 rounded-xl border border-[var(--cabi-hairline)] bg-[var(--cabi-surface-2)] px-5 text-sm font-semibold"><RefreshCw size={15} /> Try again</button>
          </section>
        )}
        ...
        {view.phase === "ready" && (
          <div className="mt-8 space-y-4">
            <section className="glass rounded-2xl p-5">
              <div className="flex flex-wrap items-start justify-between gap-4">
                <div className="min-w-0">
                  <p className="text-[10px] font-semibold uppercase tracking-[.18em] text-[var(--cabi-primary)]">Connected wallet</p>
                  <div className="mt-2 flex items-center gap-2">
                    <span className="truncate font-mono text-sm text-[var(--cabi-text-secondary)]" title={view.snapshot.address}>{shortAddress(view.snapshot.address, 10, 8)}</span>
                    <button onClick={() => void copy(view.snapshot.address)} className="focus-ring grid h-8 w-8 place-items-center rounded-lg text-[var(--cabi-text-muted)] hover:bg-[var(--cabi-surface-3)] hover:text-white" aria-label="Copy wallet address">
                      {copied ? <Check size={14} className="text-emerald-300" /> : <Copy size={14} />}
                    </button>
                  </div>
                  <p className="mt-2 text-xs text-[var(--cabi-text-muted)]">{view.snapshot.chainName}</p>
                </div>
                <div className="flex items-center gap-2">
                  <button onClick={() => void load({ fresh: true })} className="focus-ring inline-flex h-9 items-center gap-2 rounded-xl border border-[var(--cabi-hairline)] bg-[var(--cabi-surface-2)] px-3.5 text-xs font-semibold text-[var(--cabi-text-secondary)] hover:bg-[var(--cabi-surface-3)]">
                    <RefreshCw size={13} /> Refresh
                  </button>
                  {view.snapshot.explorerUrl && (
                    <a href={view.snapshot.explorerUrl} target="_blank" rel="noopener noreferrer" className="focus-ring inline-flex h-9 items-center gap-2 rounded-xl border border-[var(--cabi-hairline)] bg-[var(--cabi-surface-2)] px-3.5 text-xs font-semibold text-[var(--cabi-text-secondary)] hover:bg-[var(--cabi-surface-3)]">
                      Explorer <ExternalLink size={12} />
                    </a>
                  )}
                </div>
              </div>
              <p className="mt-4 text-[10px] text-[var(--cabi-text-faint)]">
            ...
            {view.snapshot.native && (
              <section className="glass rounded-2xl p-5">
                <p className="text-[10px] font-semibold uppercase tracking-[.18em] text-[var(--cabi-primary)]">Native balance</p>
                <p className="mt-3 text-3xl font-semibold tracking-[-.03em]">{view.snapshot.native.formatted} <span className="text-lg text-[var(--cabi-text-secondary)]">{view.snapshot.native.symbol}</span></p>
              </section>
            )}

            <section className="glass rounded-2xl p-5">
              <p className="text-[10px] font-semibold uppercase tracking-[.18em] text-[var(--cabi-primary)]">Tokens</p>
              {view.snapshot.tokens.length === 0 ? (
                <p className="mt-3 text-sm leading-7 text-[var(--cabi-text-muted)]">
                ...
                <ul className="mt-4 space-y-2">
                  {view.snapshot.tokens.map((token) => (
                    <li key={token.address} className="flex flex-wrap items-center justify-between gap-3 rounded-2xl border border-[var(--cabi-hairline)] bg-[var(--cabi-surface-1)] p-4">
                      <div className="min-w-0">
                        <p className="text-sm font-medium text-white">{token.name} <span className="text-[var(--cabi-text-secondary)]">${token.symbol}</span></p>
                        <p className="mt-1 break-all font-mono text-[10px] text-[var(--cabi-text-muted)]">{shortAddress(token.address, 8, 6)}</p>
                      </div>
                      <div className="flex items-center gap-3">
                        <p className="text-lg font-semibold">{token.formatted}</p>
                        {token.verifiedUrl && (
                          <a href={token.verifiedUrl} target="_blank" rel="noopener noreferrer" className="focus-ring inline-flex h-9 items-center gap-1.5 rounded-xl bg-[var(--cabi-primary)] px-3 text-[11px] font-semibold text-[var(--cabi-on-primary)]">
            ...
            <p className="px-1 text-[11px] leading-5 text-[var(--cabi-text-faint)]">
