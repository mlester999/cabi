    ...
    <main className="cabi-noise min-h-[100dvh] overflow-x-hidden bg-transparent text-white">
      <div className="mx-auto w-full max-w-[720px] px-4 py-6 sm:px-7 sm:py-9">
        <header className="flex items-center gap-3">
          <Link href="/leaderboard" className="focus-ring grid h-11 w-11 shrink-0 place-items-center rounded-xl border border-[var(--cabi-hairline)] bg-[var(--cabi-surface-2)] text-[var(--cabi-text-secondary)] hover:text-white" aria-label="Back to the leaderboard">
            <ArrowLeft size={18} />
          </Link>
        ...
        <section className="glass mt-6 rounded-2xl p-5 sm:p-6">
          <div className="flex flex-wrap items-center gap-4">
            ...
            <div className="min-w-0 flex-1">
              ...
              <div className="mt-1.5 flex flex-wrap items-center gap-2">
                <RankBadge tier={tier} />
                {profile.seasonLabel ? <span className="text-[11px] text-[var(--cabi-text-muted)]">{profile.seasonLabel}</span> : null}
              </div>
            </div>
          </div>
          ...
          <div className="mt-6">
            <div className="flex items-end justify-between gap-3">
              <div>
                <p className="text-[10px] font-semibold uppercase tracking-[.16em] text-[var(--cabi-primary)]">This season</p>
        ...
        <section className="mt-4 grid gap-3 sm:grid-cols-2">
          <div className="glass rounded-2xl p-5">
            <p className="text-[10px] font-semibold uppercase tracking-[.16em] text-[var(--cabi-text-muted)]">Lifetime</p>
            ...
            {profile.bestLeaderboardPosition ? (
              <p className="mt-1 text-[11px] text-[var(--cabi-text-secondary)]">Best finish #{profile.bestLeaderboardPosition}</p>
            ) : null}
          </div>
          <div className="glass rounded-2xl p-5">
            <p className="text-[10px] font-semibold uppercase tracking-[.16em] text-[var(--cabi-text-muted)]">On Cabi since</p>
            ...
            {profile.bestRankTier ? (
              <p className="mt-1 text-[11px] text-[var(--cabi-text-secondary)]">Best rank {tierByNumber(profile.bestRankTier).label}</p>
            ) : null}
          </div>
        </section>
          ...
          <section className="glass mt-4 rounded-2xl p-5">
            ...
            <ul className="mt-3 flex flex-wrap gap-2">
              {profile.achievements.map((code) => (
                <li key={code} className="rounded-full border border-[var(--cabi-hairline)] bg-[var(--cabi-surface-2)] px-3 py-1.5 text-[11px] font-medium text-[var(--cabi-text-secondary)]">
        ...
        {profile.showBondPublicly ? (
          <p className="mt-4 text-center text-[11px] text-[var(--cabi-text-faint)]">This member shares their Cabi bond badge.</p>
        ) : null}
