import { AdminRankingPanel } from "@/components/admin/admin-ranking-panel";
import type { Metadata } from "next";

export const metadata: Metadata = { title: "Leaderboard", robots: { index: false, follow: false } };

export default function AdminLeaderboardPage() {
  return (
    <div className="mx-auto w-full max-w-[1100px] p-5 sm:p-8">
      <header className="mb-6">
        <h1 className="text-xl font-bold tracking-[-0.02em]">Leaderboard</h1>
        <p className="mt-1.5 text-xs leading-6 text-[#8e889b]">
          Weekly and monthly periods, XP ledger review, rank thresholds, and manual reward tracking. Rank comes from lifetime activity, never token ownership.
        </p>
      </header>
      <AdminRankingPanel />
    </div>
  );
}
